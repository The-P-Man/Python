...
shared = list(set(coords1).intersection(set(coords2)))
print('The number of shared coordinates', len(shared))


def get_manhattan(coords):
    '''Return the Manhattan Distances for a list of coordinates'''
    return [abs(x) + abs(y) for x, y in coords]


distance = get_manhattan(shared)
print('The nearest intersection', min(distance))
print('The intersection coordinates', shared[distance.index(min(distance))])
steps = [coords1.index(coord) + coords2.index(coord) + 2 for coord in shared]
print('The minimum number of steps', min(steps))
print('The minimum steps coordinates', shared[steps.index(min(steps))])
